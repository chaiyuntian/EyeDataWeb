...
Array.prototype.desInsert=function(item,proName){
  if(this.indexOf(item)>=0)return false;
    ...
    if(this[index+1]&&this[index][proName]<this[index+1][proName]){
        this.push(item);this.des(proName);
    }
    else
...
Array.prototype.max=function(propertyName){
  if(typeof propertyName=="function"){
    ...
    this.forEach(function(e){var v=propertyName(e);if(v>e) m=v;});
  }
  else{
    var m=this[0][propertyName];
    this.forEach(function(e){ if(e[propertyName]>m)m= e[propertyName];});
  }
  return m;
};
...
Object.defer=window.Q? Q.defer:(function(){
  ...
  function defer() {
    ...
    d.promise={
      then: function (_callback, _errback) {
        ...
        else  enqueue(function () { value.then(callback, errback);});
  ...
  function reject (reason) {
   ...
   p.then= function (callback, errback) {
        var result = defer();
        enqueue(function () {result.resolve(errback(reason)); });
        return result.promise;
      };
   return p;
 }
...
Function.prototype.inherit=function(objPrototype,proArray,baseConstructor){
  ...
  proArray.forEach(function(p){base.addProperty(p);});
  this.prototype=base;
  return this;
};
